...
def f1(target, predict):
    ...
    '''
    with torch.no_grad():
        pre = precision(target, predict)
        re = recall(target, predict)
    return 2*(pre*re)/(pre+re) if pre+re !=0 else (pre+re)*0
...
def precision(target, predict):
    ...
    with torch.no_grad():
        ...
        t_p_count = (target_values * predicted).sum()
        f_p_count = (target_values * (1-predicted)).sum()
    return t_p_count/(t_p_count+f_p_count) if t_p_count+f_p_count != 0 else t_p_count * 0
...
def recall(target, predict):
    ...
    with torch.no_grad():
        ...
        predicted = torch.where(predict > 0.5, ones, zeros)
        t_p_count = (target_values * predicted).sum()
        f_n_count = ((1 - target_values) * predicted).sum()

    return t_p_count/(t_p_count+f_n_count) if t_p_count+f_n_count != 0 else t_p_count * 0
...
def get_arguments():
    ...
    '''
    save_path = 'D:\\dataset\\Droplet\\train'
    data_set_path = 'D:\\dataset\\Droplet'
    parser = argparse.ArgumentParser()
    parser.add_argument('--save', '-s', nargs='+', help='save path', default=[save_path], dest='save_path')
    parser.add_argument('--epoch', '-e', nargs='+', help='epoch count', default=[100], dest='epoch', type=int)
...
def checkpoint(model, train_info):
    '''
        ...
        best 모델은 실행 경로에 생성.
    '''
    v_acc = train_info.validate_acc
    v_acc[0] = v_acc[0].detach().cpu().numpy()
    v_acc[1] = v_acc[1].detach().cpu().numpy()
    v_acc[2] = v_acc[2].detach().cpu().numpy()
    print('validate precision : ', v_acc[0], ' max precision :',  checkpoint.max_acc[0])
    print('validate recall : ', v_acc[1], ' max recall : ',  checkpoint.max_acc[1])
    print('validate f1 : ', v_acc[2], ' max f1 : ',  checkpoint.max_acc[2])
    model_save_path = train_info.save_path+'//model'
    os.makedirs(model_save_path, exist_ok=True)
    if v_acc[2] > checkpoint.max_acc[2]:
        print('*********saved new best model*********')
        checkpoint.max_acc[0] = v_acc[0]
        checkpoint.max_acc[1] = v_acc[1]
        checkpoint.max_acc[2] = v_acc[2]
        torch.save(model.state_dict(), '{}//model{}.dict'.format(model_save_path, v_acc[2]))
        traced_cell = torch.jit.trace(model, torch.rand(1, 3, 256, 256).type(torch.FloatTensor).cuda())
        traced_cell.save('{}//model{}.zip'.format(model_save_path, v_acc[2]))


checkpoint.max_acc = np.asarray([0,0,0],  dtype=np.float)
